...
void loop()
{
 ...
 //random values are taken 1 std dev up and down plus the mean out of roughly 1000 values tested.
  ...
  dtostrf(volt,5,4,sendvolt);
  dtostrf(amp,6,5,sendamp);
//   sendtime = timezd+15;
 
  // Generate the char string to be sent:
  watt=volt*amp;
  dtostrf(watt,7,6,sendwatt);
  sprintf(sendbuffer,"%s,%s,%s",sendvolt,sendamp,sendwatt);
  ...
  // If there is any serial input, add it to the buffer:
    ...
    //{
      ...
      //for (byte i = 0; i < sendlength; i++) //when I commented out this part and
        Serial.println(sendbuffer);             //removed the [i] from this line it began incrementing the message correctly
      //Serial.println("]");

...
void loop()
{
  ...
  static int sendlength = 9;

  // Generate the char string to be sent:
  //seed = 325
  mwh_value=seed++;

  dtostrf(mwh_value,10,3,sendbuffer);
  // SENDING
  ...
  // If there is any serial input, add it to the buffer:
    ...
    //{
      ...
      //for (byte i = 0; i < sendlength; i++) //when I commented out this part and
        Serial.print(sendbuffer);             //removed the [i] from this line it began incrementing the message correctly
      Serial.println("]");
      ...
      sendlength = 8; // reset the packet adjusted to 8 from 0, before when it was zero the received message would be blank
      Blink(LED,10);
    //}

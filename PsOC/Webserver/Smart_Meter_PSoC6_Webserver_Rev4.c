...
void application_start( void )
{
    ...
    WPRINT_APP_INFO(("*******************************************************\n"));
    wiced_uart_transmit_bytes( STDIO_UART, "Page Ready at\n\r", sizeof( "Page Ready at\n\r" ) - 1 );
    wiced_uart_transmit_bytes( STDIO_UART, &buffer, sizeof( buffer ) - 1 );
    UG_FontSelect( &HEADING_FONT );
...
int32_t processUsageUpdate( const char* url_path, const char* url_parameters, wiced_http_response_stream_t* stream, void* arg, wiced_http_message_body_t* http_data )
{
    ...
    wiced_uart_transmit_bytes( STDIO_UART, "PageAccess\n\r", sizeof( "PageAccess\n\r" ) - 1 );
    ...
    wiced_uart_transmit_bytes( STDIO_UART, &tempCharBuffer, sizeof( tempCharBuffer ) - 1 );
    wiced_uart_transmit_bytes( STDIO_UART, "\n\r", sizeof( "\n\r" ) - 1 );
    tempCharBufferLength = sprintf(tempCharBuffer,"%s",recdstring);

    StartOfData = strchr(tempCharBuffer,'[');
    if (StartOfData != NULL){
    voltage[0] = StartOfData[1];
    voltage[1] = StartOfData[2];
    voltage[2] = StartOfData[3];
    voltage[3] = StartOfData[4];
    voltage[4] = StartOfData[5];
    voltage[5] = StartOfData[6];
    voltage[6] = '\0';
    usage[0] = StartOfData[15];
    usage[1] = StartOfData[16];
    usage[2] = StartOfData[17];
    usage[3] = StartOfData[18];
    usage[4] = StartOfData[19];
    usage[5] = StartOfData[20];
    usage[6] = StartOfData[21];
    usage[7] = '\0';
    }
    else
    {
    ...
    wiced_time_get_iso8601_time( &usageData.timestamp );

           curr_time = &usageData.timestamp;
    ...
    tempCharBufferLength = sprintf(tempCharBuffer, "%s", usage);
    ...
    tempCharBufferLength = sprintf(tempCharBuffer, "%s", voltage);
    wiced_http_response_stream_write(stream, tempCharBuffer, tempCharBufferLength );

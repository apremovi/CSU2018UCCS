...
int32_t processUsageUpdate( const char* url_path, const char* url_parameters, wiced_http_response_stream_t* stream, void* arg, wiced_http_message_body_t* http_data )
{
    ...
    wiced_http_response_stream_write_resource( stream, &resources_apps_DIR_Smart_Meter_PSoC6_Webserver_DIR_data_html_temp_start );
    tempCharBufferLength = sprintf(tempCharBuffer, "%d", usageData.usage);
    wiced_http_response_stream_write(stream, tempCharBuffer, tempCharBufferLength );
    wiced_http_response_stream_write_resource(  stream, &resources_apps_DIR_Smart_Meter_PSoC6_Webserver_DIR_data_html_temp_end);
